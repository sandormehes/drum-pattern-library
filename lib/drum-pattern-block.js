...
function renderPattern(lines) {
  ...
  const loopOptions = Array.from({ length: stepCount / 16 }, (_, index) => {
    const bars = index + 1
    return `<option value="${bars * 16}"${bars * 16 === stepCount ? ' selected' : ''}>${bars} ${bars === 1 ? 'bar' : 'bars'}</option>`
  }).join('')
  const mixerRows = tracks.map(({ label }, index) => `<div class="drum-pattern-track-control">
<strong>${escapeHtml(label)}</strong>
<label><input type="checkbox" data-track-mute="${index}"> Mute</label>
<label><input type="checkbox" data-track-solo="${index}"> Solo</label>
<button class="drum-pattern-stem" type="button" data-track-stem="${index}">Download stem</button>
</div>`).join('')
...
<label class="drum-pattern-loop">Loop <select aria-label="Loop length">${loopOptions}</select></label>
<label class="drum-pattern-click"><input type="checkbox"> Metronome</label>
<label class="drum-pattern-kit">Sound set <select aria-label="Sound set"><option value="synth">Built-in synthesis</option><option value="drum-lab">Drum Lab kit</option></select></label>
...
<details class="drum-pattern-mixer"><summary>Mix and download stems</summary><div class="drum-pattern-track-controls">${mixerRows}</div></details>
